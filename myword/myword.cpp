...
void MyWord::createActions()
{
    ...
    undoAct = new QAction(QIcon(rsrcPath + "/editundo.png"),tr("撤销(&U)"), this);
       ...
       /*"格式"主菜单动作集*/
           ...
           tableAct = new QAction(QIcon(rsrcPath + "/edittable.png"),tr("表格(&C)"), this);
           tableAct->setToolTip("表格");
           tableAct->setStatusTip(tr("设置表格"));
           connect(tableAct, SIGNAL(triggered()), this, SLOT(textTable()));
           /*"窗口"主菜单动作集*/
...
void MyWord::createToolBars()
{
    ...
    formatToolBar->addAction(tableAct);
    //组合选择栏
...
void MyWord::updateMenus()
{
    ...
    tableAct->setEnabled(hasSelection);
}
MyChild *MyWord::activeMyChild()
{
    //若有活动窗口，则将其内的中心部件转换为MyChild类型；若没有，则直接返回0
...
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("打开"), QString(), tr("HTML 文档 (*.htm *.html);;所有文件 (*.*)"));
    if (!fileName.isEmpty()) {
...
void MyWord::textTable(){
    if (activeMyChild())
        activeMyChild()->textTable();
}
 MyWord::~MyWord(){
}
